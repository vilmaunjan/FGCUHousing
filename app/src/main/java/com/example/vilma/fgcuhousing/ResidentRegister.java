...
public class ResidentRegister extends AppCompatActivity {
    ...
    public void buttonOnClick(View v) {
        ...
        if (v == findViewById(R.id.btnSubmit)) {
            ...
            if(validInput == true){
                validInput = nameEntry.matches("[a-zA-Z]+ ");
            }
            //email must be in format ****@eagle.fgcu.edu
            //in full version must verify email though an Email.
            if(validInput == true) {
                validInput = emailEntry.endsWith("@eagle.fgcu.edu");
            }
            //passwords must match
            if(validInput == true) {
                validInput = passwordEntry.equals(passwordVerify);
            }
